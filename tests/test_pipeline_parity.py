...
def test_full_profile_matrix_defaults_to_three_arm_mask_and_optimization_isolation() -> None:
    ...
    assert profiles[2].mask_source == "candidate"
    assert profiles[2].parallel_pose_densepose is True
    assert profiles[2].parallel_post_fusion is True
    assert dict(profiles[2].environment_overrides) == {
...
def test_candidate_profile_overlaps_analysis_and_post_fusion_stages(tmp_path: Path) -> None:
    ...
    result = run_pipeline_profile(
        tmp_path,
        PIPELINE_BENCHMARK_PROFILES["downstream_candidate_optimized"],
...
def test_production_profile_reports_stage_timings_without_paths(tmp_path: Path) -> None:
    ...
    assert result["stages"]["production_full_pipeline"]["steps"] == [
        {"stage": "mask", "status": "complete", "elapsed_seconds": 12.5, "frame_count": 260}
    ]
    assert "/private" not in json.dumps(result)


def test_production_profiles_apply_exact_deploy_configs_and_restore_environment(
    tmp_path: Path,
...
def test_full_benchmark_response_is_bounded_and_contains_no_artifact_bytes(
    ...
    payload = {
        ...
        "scope": "full",
        "fixture_id": CANONICAL_FRAME130_FIXTURE_ID,
        "profile_ids": ["production_control", "production_candidate"],
        "assets": {
            "person_prompt_json": {
                "encoding": "base64",
...
def test_three_arm_runner_generates_candidate_mask_once_and_reuses_exact_bytes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert (
        observed_masks["downstream_candidate_control"]
        == observed_masks["downstream_candidate_optimized"]
    )
