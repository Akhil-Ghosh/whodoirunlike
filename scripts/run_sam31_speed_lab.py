...
FULL_PROFILE_MATRICES = {
    ...
    "authoritative-final-candidate": ["production_final_candidate"],
}
...
def _resolve_full_profile_ids(
    *,
    ...
    if profile_id:
        ...
        return [profile_id]
    matrix_id = profile_matrix or "sam-mask-control"
    try:
...
def _final_candidate_result_passed(status_payload: dict[str, Any]) -> bool:
    output = status_payload.get("output")
    if not isinstance(output, dict) or output.get("parity_passed") is not True:
        return False
    runtime_gates = output.get("runtime_gates")
    if not isinstance(runtime_gates, dict):
        return False
    exact_loader = runtime_gates.get("exact_cv2_loader")
    return isinstance(exact_loader, dict) and exact_loader.get("passed") is True
...
def main() -> int:
    ...
    if profile_ids == ["production_final_candidate"] and args.artifact_sink_file is None:
        parser.error("production_final_candidate requires --artifact-sink-file")
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if status in {"COMPLETED", "FAILED", "CANCELLED", "TIMED_OUT"}:
                ...
                if status != "COMPLETED":
                    return 1
                if profile_ids == ["production_final_candidate"] and not (
                    _final_candidate_result_passed(status_payload)
                ):
                    print(
                        "Final candidate completed without passing parity and exact-loader gates.",
                        file=sys.stderr,
                    )
                    return 1
                return 0
