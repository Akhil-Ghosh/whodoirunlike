...
PIPELINE_BENCHMARK_PROFILES = {
    ...
    "production_final_candidate": PipelineBenchmarkProfile(
        profile_id="production_final_candidate",
        execution_mode="production_full_pipeline",
        mask_source="production",
        parallel_mask_presentation=True,
        parallel_pose_densepose=True,
...
def evaluate_exact_loader_gate(profile_result: dict[str, Any]) -> dict[str, Any]:
    pipeline_stage = (profile_result.get("stages") or {}).get(
        "production_full_pipeline",
        {},
    )
    steps = pipeline_stage.get("steps") if isinstance(pipeline_stage, dict) else None
    mask_step = next(
        (
            step
            for step in (steps if isinstance(steps, list) else [])
            if isinstance(step, dict) and step.get("stage") == "mask"
        ),
        {},
    )
    loader_seconds = mask_step.get("exact_cv2_loader_seconds")
    seconds_valid = (
        isinstance(loader_seconds, (int, float))
        and not isinstance(loader_seconds, bool)
        and math.isfinite(float(loader_seconds))
        and float(loader_seconds) > 0.0
    )
    checks = {
        "enabled": mask_step.get("exact_cv2_loader_enabled") is True,
        "attempted": mask_step.get("exact_cv2_loader_attempted") is True,
        "used": mask_step.get("exact_cv2_loader_used") is True,
        "nonzero_timing": seconds_valid,
        "required_concurrency_one": mask_step.get(
            "exact_cv2_loader_required_concurrency"
        )
        == 1,
        "configured_concurrency_one": mask_step.get(
            "exact_cv2_loader_configured_concurrency"
        )
        == 1,
        "concurrency_ready": mask_step.get("exact_cv2_loader_concurrency_ready")
        is True,
    }
    return {
        ...
        "checks": checks,
        "measurements": {
            field: mask_step.get(field) for field in _EXACT_LOADER_RESULT_FIELDS
        },
    }
...
def _full_runtime_metadata() -> dict[str, Any]:
    ...
    if image_role == "final_candidate":
        result["final_candidate_identity"] = validate_final_candidate_identity(
            commit=str(result["candidate_commit"]),
            image_digest=str(result["candidate_image_digest"]),
            image_reference=str(result["final_candidate_image_reference"]),
        )
        result["densepose_weights"] = verify_densepose_weights()
        result["quality_override_environment"] = {
            name: os.getenv(name) for name in FINAL_FORBIDDEN_QUALITY_ENVIRONMENT
        }
...
def _create_handoff_bundle(
    *,
    ...
    manifest = {
        ...
        "final_candidate_image_reference": runtime.get("final_candidate_image_reference"),
        "final_candidate_identity": runtime.get("final_candidate_identity"),
...
def run_full_pipeline_benchmark(
    ...
    *,
    ...
    is_candidate_handoff = image_role in {"schedule_only", "final_candidate"}
    ...
    if sink is not None:
        ...
        elif image_role == "final_candidate":
            if profile_ids != ["production_final_candidate"]:
                raise ValueError(
                    "Final-candidate handoff requires production_final_candidate."
                )
            candidate_commit = runtime.get("candidate_commit")
            candidate_digest = runtime.get("candidate_image_digest")
            final_provenance = {
                "identity": isinstance(runtime.get("final_candidate_identity"), dict)
                and runtime["final_candidate_identity"].get("passed") is True,
                "processor_version": runtime.get("processor_version") == candidate_commit,
                "base_processor_commit": runtime.get("base_processor_commit")
                == candidate_commit,
                "base_processor_image_digest": runtime.get("base_processor_image_digest")
                == candidate_digest,
                "code_overlay_commit": runtime.get("code_overlay_commit")
                == candidate_commit,
                "code_overlay_source": runtime.get("code_overlay_source") == "base_image",
                "code_overlay_reference_image_digest": runtime.get(
                    "code_overlay_reference_image_digest"
                )
                == candidate_digest,
                ...
                "dependency_base_commit": runtime.get("dependency_base_commit")
                == candidate_commit,
                "dependency_base_image_digest": runtime.get(
                    "dependency_base_image_digest"
                )
                == candidate_digest,
                "base_contract": isinstance(runtime.get("base_contract"), dict)
                and runtime["base_contract"].get("passed") is True
                and runtime["base_contract"].get("image_role") == "final_candidate",
                "sam31_mask_backend": os.getenv(
                    "WHODOIRUNLIKE_MASK_BACKEND", ""
                ).strip().lower()
                == "sam31_gpu",
                "mmpose_cpu": os.getenv("MMPOSE_DEVICE", "").strip().lower() == "cpu",
                "rtmw_cpu_runtime": os.getenv("RTMW_RUNTIME_BACKEND", "").strip().lower()
                == "onnxruntime",
                "quality_overrides_absent": not any(
                    str(value or "").strip()
                    for value in (
                        runtime.get("quality_override_environment") or {}
                    ).values()
                ),
                "densepose_weights_environment": os.getenv(
                    "DENSEPOSE_WEIGHTS", ""
                ).strip()
                == str(DENSEPOSE_WEIGHTS_PATH),
                "densepose_weights_verified": isinstance(
                    runtime.get("densepose_weights"), dict
                )
                and runtime["densepose_weights"].get("passed") is True,
            }
            if not all(final_provenance.values()):
                failed = [name for name, passed in final_provenance.items() if not passed]
                raise ValueError(
                    "Final-candidate handoff provenance failed: " + ", ".join(failed)
                )
        else:
            raise ValueError(
                "Artifact handoff requires a control, schedule-only, or final-candidate "
                "image role."
            )
    ...
    with _full_benchmark_workspace() as (temp_root, bundle_id, artifacts_persisted):
        ...
        runtime_gates: dict[str, Any] = {
            "exact_cv2_loader": {"status": "not_required"},
        }
        if image_role == "final_candidate":
            runtime_gates["exact_cv2_loader"] = evaluate_exact_loader_gate(
                profile_results[0]
            )
        ...
        if image_role == "final_candidate":
            parity_components.append(
                bool(runtime_gates["exact_cv2_loader"].get("passed"))
            )
        ...
        result: dict[str, Any] = {
            ...
            "timing_note": (
                "Profiles execute in listed order in one process; authoritative final parity "
                "uses one profile and an immutable cross-image control handoff."
            ),
