...
def test_cli_resolves_full_profile_selection_and_named_matrix() -> None:
    ...
    assert script._resolve_full_profile_ids(
        profile_id=None,
        profile_matrix="sam-mask-control",
    ) == [
    ...
    assert script._resolve_full_profile_ids(
        profile_id=None,
        profile_matrix="authoritative-final-candidate",
    ) == ["production_final_candidate"]
...
def test_cli_final_result_requires_parity_and_exact_loader_gate() -> None:
    script = _load_speed_lab_script()
    passing = {
        "output": {
            "parity_passed": True,
            "runtime_gates": {"exact_cv2_loader": {"passed": True}},
        }
    }

    assert script._final_candidate_result_passed(passing) is True
    passing["output"]["runtime_gates"]["exact_cv2_loader"]["passed"] = False
    assert script._final_candidate_result_passed(passing) is False
