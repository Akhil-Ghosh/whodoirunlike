...
The Processor delivers callbacks through a bounded pool of six workers by default; `WHODOIRUNLIKE_TELEMETRY_DELIVERY_WORKERS` can tune the pool from one to eight. Production measurements showed that the former single sender delivered only about 0.52 events per second and could leave terminal telemetry more than three minutes behind processing. Concurrent delivery preserves the existing per-event contract and exact stage/span detail while removing that head-of-line backlog. The deployed template also bounds individual callback and report writes with `WHODOIRUNLIKE_TELEMETRY_DELIVERY_TIMEOUT_SECONDS=10` and `WHODOIRUNLIKE_REPORT_TIMEOUT_SECONDS=10`. `WHODOIRUNLIKE_TELEMETRY_DRAIN_TIMEOUT_SECONDS` remains a final correctness deadline; if it is exhausted, the Processor writes a sanitized error containing the run and attempt IDs plus delivery counters, never event bodies, callback URLs, headers, or secrets.
